...
-- Uncached: String: O(N*2), table: O(log N), avl: O(log N)
...
-- Results:
-- Uncached: avl: 0.01s, table: 0.01s, string: 0.01s
-- Cached: avl: 0.001s, table: 0.001s, string: 0.001s
...
-- Cached: avl: O(1000), table: O(1000), string: O(1000) - Only the records we want are processed.                          
for _, dataStructure in ipairs(dataStructures) do
    local rankStore = RankStore.GetRankStore(rankStoreName, numBuckets, maxBucketsSize, lazySaveTime, true, dataStructure, "base91")
    local start = os.clock()
